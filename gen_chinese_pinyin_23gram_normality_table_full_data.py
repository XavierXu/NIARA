...
#                if counter < 20:
                if counter < 1000000:
                    ...
                    four_gram_results = calc_ngram(string, 4)
                    n_gram_results = uni_gram_results + bi_gram_results + tri_gram_results + four_gram_results
                    yield n_gram_results, string, family, uni_gram_results, bi_gram_results, tri_gram_results, four_gram_results
...
#                    print 'line is %s'%line
                    ...
                    four_gram_results =calc_ngram(string, 4)
                    n_gram_results = uni_gram_results + bi_gram_results + tri_gram_results + four_gram_results
                    yield n_gram_results, string, 'DGA', uni_gram_results, bi_gram_results, tri_gram_results, four_gram_results
...
#    dic_Pb = {}
    ...
    for each in generator_table_123gram_good('Alexa_top_1M_country'):
        ...
        for item in each[4]:    # each[4] is bi-gram list
            if item[0] in dic_chinese:    #item[0] is bi-gram letters
                tmp_2gram_count = dic_chinese[item[0]] 
            else:
                tmp_2gram_count = 0
        ...
        for item in each[5]:    # each[5] is tri-gram list
            if item[0] in dic_chinese:    #item[0] is tri-gram letters
                tmp_3gram_count = dic_chinese[item[0]] 
            else:
                tmp_3gram_count = 0
        ...
        for item in each[6]:    # each[6] is four-gram list
            if item[0] in dic_chinese:    #item[0] is tri-gram letters
                tmp_4gram_count = dic_chinese[item[0]] 
            else:
                tmp_4gram_count = 0
            tmp_sum = float(tmp_sum) + float(tmp_3gram_count)
        if len(each[5]) == 0:
            tmp_4gram_nor_score = 'N/A'
        else:
            tmp_4gram_nor_score = float(tmp_sum)/float(len(each[5]))
        ...
        dic[each[1]].append(tmp_4gram_nor_score)
        dic_good[each[1]].append(tmp_4gram_nor_score)
...
#creating bad
    ...
    for each in generator_table_123gram_bad('DGA_Data'):
        ...
        dic[each[1]].append(tmp_4gram_nor_score)
        dic_bad[each[1]].append(tmp_4gram_nor_score)
        dic[each[1]].append(each[2])  #each[2] is good/bad
...
#######
    ...
    print dic_good
    print dic_bad
    with open('234gram_chinese_normality_score.json', 'w') as outfile:
            json.dump(dic, outfile)

    with open('234gram_chinese_normality_score_Alexa.json', 'w') as outfile:
            json.dump(dic_good, outfile)

    with open('234gram_chinese_normality_score_DGA.json', 'w') as outfile:
